...
int saveMCLevel ( uint8_t* mclayers,uint8_t* mcdata,int mcxsquares,int mcysquares,int mczsquares,int cloudheightdf, int xs, int ys, int zs,char* name )
{
    ...
    int of = open ( "out.mcraw",O_BINARY | O_CREAT | O_TRUNC | O_WRONLY, S_IREAD | S_IWRITE );
    ...
    for ( uint32_t dfoffsetx=0;dfoffsetx<SQUARESPERBLOCK;dfoffsetx++ )
    {
        for ( uint32_t dfoffsety=0;dfoffsety<SQUARESPERBLOCK;dfoffsety++ )
        {
            ...
            case SHRUB_OK:
            {
                ...
                {
                    plant = vit->second;
                    snprintf ( classname,127,"%s.%s",TileClassNames[tileTypeTable[tiletype].c],plant );
                }
                else
                {
            ...
            case RAMP:
            {
                if ( classname[0]=='\0' )
                {
                    ...
                    snprintf ( classname,127,"%s%s",TileClassNames[tileTypeTable[tiletype].c],dir );
                }
            }
            break;
            case STAIR_UP:
            ...
            case STAIR_UPDOWN:
            {
                if ( classname[0]=='\0' )
                {
                    snprintf ( classname,127,"%s%d",TileClassNames[tileTypeTable[tiletype].c],zcount%4 );//z$4 is the level height mod 4 so you can do spiral stairs or alterante sides if object defination of 0 = 2 and 1 = 3
                }
            }
            break;
            default:
            ...
            {
                snprintf ( classname,127,"%s.%s","treetop",plant );
...
//                          printf("Not replacing a building with a stockpile\n");
    ...
    //snprintf(name,255,"out %d-%s-%d.mclevel",year,Months[month],day);
    //name[255]='\0';
